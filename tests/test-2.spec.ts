...
for (let i = 1; i < 2; i++) {
  test.describe('Performance with one unified command ' + i, async () => {
    test(i + ` Should load ${url} page in less than 2 second`, async ({ page, collector }) => {
    ...
    test(i + ` Should load ${url} page with timeToFirstByte less than 500ms`, async ({ page, collector }) => {
      const metrics = await collector.collectMetrics(page)
      fs.appendFileSync('results.json', JSON.stringify(metrics) + '\n')
    ...
    test(i + ` Should load ${url} page with resourceTiming less than 500ms`, async ({ page, collector }) => {
      const metrics = await collector.collectMetrics(page)
      fs.appendFileSync('results.json', JSON.stringify(metrics) + '\n')
    ...
    test(i + ` Should load ${url} page with size less than 1.5 MB`, async ({ page, collector }) => {
      const metrics = await collector.collectMetrics(page)
      fs.appendFileSync('results.json', JSON.stringify(metrics) + '\n')
      expect(metrics.totalBytes, 'Total bytes is less than 1.5 MB').toBeLessThan(1024 * 1024 * 1.5)
    })
    test(i + ` Should measure paint timings for ${url}`, async ({ page, collector }) => {
      const metrics = await collector.collectMetrics(page)
      fs.appendFileSync('results.json', JSON.stringify(metrics) + '\n')
    ...
    test(i + ` Should measure largestContentfulPaint for ${url}`, async ({ page, collector }) => {
      const metrics = await collector.collectMetrics(page)
      fs.appendFileSync('results.json', JSON.stringify(metrics) + '\n')
    ...
    test(i + ` Should measure cumulativeLayoutShift for ${url}`, async ({ page, collector }) => {
      const metrics = await collector.collectMetrics(page)
      fs.appendFileSync('results.json', JSON.stringify(metrics) + '\n')
    ...
    test(i + ` Should measure totalBlockingTime for ${url}`, async ({ page, collector }) => {
      const metrics = await collector.collectMetrics(page)
      fs.appendFileSync('results.json', JSON.stringify(metrics) + '\n')
